...
public class BookingTransitions {
    ...
    private static final Logger log = LoggerFactory.getLogger(BookingTransitions.class);
    ...
    public BookingTransitions(JdbcTemplate jdbc, OutboxWriter outbox, DbClock clock, TicketIssuer ticketIssuer,
    ...
    private void publishStatus(UUID bookingId, String status, String refundState) {
        events.publishEvent(new BookingSseController.BookingStatusChanged(bookingId, status, refundState));
    }

    public record BookingRow(UUID id, UUID userId, UUID eventId, UUID tierId, Integer quantity, String status,
     ...
     */
    @Transactional
    public BookingDtos.PaymentIntentView createPaymentIntent(UUID bookingId) {
        ...
        BookingDtos.IntentRow existing = jdbc.query("""
                ...
                """, (rs, i) -> new BookingDtos.IntentRow(rs.getObject("id", UUID.class), rs.getInt("attempt_no"),
                ...
                .orElse(null);
        if (existing != null) {
            return new BookingDtos.PaymentIntentView(existing.id(), existing.attemptNo(), existing.state(),
        ...
        return new BookingDtos.PaymentIntentView(intentId, attemptNo, "CREATED", totalMinor(booking), 0L,
    ...
    private void cancelBeforePayment(BookingRow booking) {
        ...
        jdbc.update("""
                ...
                WHERE id = ?
                """, booking.id());
        BookingDtos.IntentRow active = activeIntent(booking.id());
        if (active != null) {
    ...
    private void cancelConfirmed(BookingRow booking, boolean eventCancelled) {
        ...
        BookingDtos.IntentRow intent = activeIntent(booking.id());
    ...
    private BookingDtos.IntentRow activeIntent(UUID bookingId) {
        return jdbc.query("""
                SELECT id, attempt_no, state, requested_amount_minor, captured_amount_minor, currency,
                       provider_key, active FROM payment_intents WHERE booking_id = ? AND active = TRUE
                """, (rs, i) -> new BookingDtos.IntentRow(rs.getObject("id", UUID.class), rs.getInt("attempt_no"),
