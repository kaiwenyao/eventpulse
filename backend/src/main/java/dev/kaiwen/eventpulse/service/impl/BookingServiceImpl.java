...
import dev.kaiwen.eventpulse.dto.BookingDtos.RefundView;
import dev.kaiwen.eventpulse.dto.BookingDtos.TicketView;
import dev.kaiwen.eventpulse.dto.BookingDtos.ViewRow;
...
public class BookingServiceImpl implements BookingService {
    ...
    /** Proxied self reference so read paths keep their transaction boundaries. */
    private final BookingService self;

    public BookingServiceImpl(JdbcTemplate jdbc, IdempotencyService idempotency, BookingTransitions transitions,
            OutboxWriter outbox, AppProperties properties, DbClock clock,
    ...
    public BookingView createBooking(UUID userId, String rawIdempotencyKey, CreateBookingRequest request) {
        ...
        outbox.append("booking", bookingId, dev.kaiwen.eventpulse.outbox.OutboxWriter.TOPIC_BOOKING, "booking.created", Map.of(
    ...
    @Override
    public List<UUID> listBookingIds(UUID userId) {
        return jdbc.queryForList("SELECT id FROM bookings WHERE user_id = ? ORDER BY created_at DESC",
                UUID.class, userId);
    }
    ...
    public PaymentIntentView payBooking(UUID userId, UUID bookingId, String rawIdempotencyKey) {
        ...
        PaymentIntentView intent = transitions.createPaymentIntent(bookingId);
    ...
    public BookingView cancelBooking(UUID userId, UUID bookingId, String rawIdempotencyKey, CancelRequest request) {
        IdempotencyService.Fingerprint fingerprint = idempotency.claim(userId, "bookings:cancel",
                rawIdempotencyKey == null ? "" : rawIdempotencyKey, Map.of("bookingId", bookingId.toString()));
        requireOwner(userId, bookingId);
        transitions.cancel(userId, bookingId, false, "user");
        BookingView view = self.getBooking(userId, bookingId);
