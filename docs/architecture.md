...
- **代码分层组织**：代码目录按技术分层（`controller` 只做请求校验/当前用户/调用 Service 接口/组装响应；`service` 为业务接口；`service.impl` 为 `@Service` 实现并承载全部 SQL；`dto` 承载请求/响应/行 record；`exception`/`config`/`common`/`batch`/`outbox`/`payment` 为基础设施）。这只是代码目录与依赖方式的调整：数据库事务边界、上述领域模块的业务职责以及"不跨模块随意写表"等关键约束全部保持不变，SQL 仍集中在对应 service 实现。
